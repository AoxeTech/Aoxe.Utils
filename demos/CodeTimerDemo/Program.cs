...
Console.WriteLine(Runner.Time("for", iteration, () =>
{
...
Console.WriteLine(Runner.Time("CopyTo", iteration, () =>
{
    var dest = new byte[srcBytes.Length];
    srcBytes.CopyTo(dest, 0);
}));

Console.WriteLine(Runner.Time("Array.Copy",iteration, () =>
{
    var dest = new byte[srcBytes.Length];
    Array.Copy(srcBytes, dest, srcBytes.Length);
}));

Console.WriteLine(Runner.Time("Clone", iteration, () =>
{
    var dest = (byte[])srcBytes.Clone();
}));

Console.WriteLine(Runner.Time("Buffer.BlockCopy", iteration, () =>
{
    var dest = new byte[srcBytes.Length];
    Buffer.BlockCopy(srcBytes, 0, dest, 0, srcBytes.Length);
}));

Console.WriteLine(Runner.Time("Span.ToArray", iteration, () =>
{
    var dest = new Span<byte>(srcBytes, 0, srcBytes.Length).ToArray();
}));

Console.WriteLine(Runner.Time("ToArray", iteration, () =>
{
